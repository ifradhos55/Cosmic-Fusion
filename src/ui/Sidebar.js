...
export function sidebar(bodies) {
  return `<aside class="sidebar">
    ...
    <nav class="body-nav" aria-label="Celestial bodies">${bodies.map((body, i) => `<button class="body-button ${body.id === 'earth' ? 'active' : ''}" data-body="${body.id}" aria-pressed="${body.id === 'earth'}"><span class="planet-dot planet-${body.id}" style="--planet-color:${body.color}"></span><span>${body.name}</span><span class="body-number">${String(i).padStart(2, '0')}</span><span class="selected-dot"></span></button>`).join('')}</nav>
